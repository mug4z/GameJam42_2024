...
var radius = 3200  # Radius of the circle

# Called when the node enters the scene tree for the first time.
func _ready():
	var num_sprites = 20  # Number of sprites to place in a circle
	for i in range(0, num_sprites):
		...
		new_sprite.position = Vector2(center_x + radius * cos(angle), center_y + radius * sin(angle)) * randf_range(0.95,1.05)
